...
App = {
    ...
    loadWeb3: async () => {
      ...
      loadContract: async () => {
        ...
        try {
          // Obtenemos las credenciales y enviamos al contrato para validar
          console.log("Usuario recibido: "+ matricula);
          const credenciales = await App.contract.ingresar(matricula, pass, {from: App.account, });
          // imprimimos 
          //console.log(credenciales.logs[0].args);
          // Obtenemos el estado de la sesion
          const estadoSesion = await App.contract.verEstadoSesion(matricula);
          console.log("El estado de la sesión es: "+estadoSesion);
          if(estadoSesion){
          // aca redicciona al dashboard
          console.log("El usuario ingresado está registrado y su sesión está abierta!")
          console.log("Ha iniciado sesión: "+ matricula);
          //window.location.href = '/dash.html';
         } else {
          console.log("El usuario ingresado no existe o no está registrado!")
         }
